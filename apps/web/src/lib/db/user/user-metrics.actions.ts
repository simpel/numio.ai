...
> {
	try {
		...
		const latestEvent = await db.metricEvent.findFirst({
			where: { type: 'user_profile_created' },
		...
		// Get all user profile events within the date range
		...
		// Count created users per day
		createdEvents.forEach((event) => {
			const dateKey = event.timestamp.toISOString().split('T')[0] as string;
