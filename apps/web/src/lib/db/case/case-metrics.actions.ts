...
export async function getCaseMetricsAction(days: number = 30): Promise<
	ActionState<
		Array<{
			date: string;
			dateLabel: string;
...
> {
	try {
		...
		// Get all case events within the date range
		...
		return {
			isSuccess: true,
			message: 'Case metrics fetched successfully',
			data: result,
		};
	} catch (error) {
